...
class Trainer(BaseTrainer):
    ...
    def _train_epoch_blip(self, epoch):
        train_loss = 0
        self.model.train()
        for batch_idx, (images, prompts, captions, cls_labels, clip_memory) in enumerate(self.train_dataloader):
            ...
            loss_lm, loss_cls, loss_cont = self.model(images, prompts, captions, cls_labels, clip_memory, self.criterion_cls, self.base_probs)
    ...
    def eval_blip(self, log):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images, captions, cls_labels, clip_memory) in enumerate(self.test_dataloader):
                ...
                clip_memory = clip_memory.to(self.device) 
                ground_truths = captions
                reports, cls_preds, _ = self.model.module.generate(images, clip_memory, sample=False, num_beams=self.args.beam_size, max_length=self.args.gen_max_len, min_length=self.args.gen_min_len)

                # 计算预测准确率
                cls_p = np.array(cls_preds) # b,18
                cls_gt = np.array(cls_labels) # b,14
                cls_p = cls_p[:,:cls_gt.shape[-1]]
                result= (cls_p == cls_gt).astype(int)     
